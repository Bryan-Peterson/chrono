...
// to be dropped and rolled on a simple flat terrain using a steady
// state LuGre friction model.
...
public:
	...
	void Calculate_dLength(double Vec1x, double Vec1y, double Vec2x, double Vec2y, double Vec3x, double Vec3y, double &Length)
	{
		int i, ii, j, jj, kk;
		double TempR;
		double TempR1;
	...
	void spline_cubic_set(int n, ChVectorDynamic<double> t, ChVectorDynamic<double> y, int ibcbeg, double ybcbeg, int ibcend, double ybcend, ChVectorDynamic<double> &ypp)
	{
		...
		for (int i = 0; i < n - 1; i++)
		{
			if (t(i + 1) <= t(i))
			{
				GetLog() << "\nSPLINE_CUBIC_SET - Fatal error!\nThe knots must be strictly increasing, but T(" << i << ") = " << t(i) << " T(" << i + 1 << ") = " << t(i + 1);
				system("pause");
			}
		}
		if (ibcbeg == 0)
	...
	void spline_cubic_val(int n, ChVectorDynamic<double> t, ChVectorDynamic<double> y, ChVectorDynamic<double> ypp, double tval, double &yval, double &ypval, double &yppval)
	{
		int left = 0;
		int right = 0;
		double dt;
		...
		yval = y(left) + dt*((y(right) - y(left)) / h - (ypp(right) / 6.0 + ypp(left) / 3.0)*h + dt*(0.5*ypp(left) + dt*((ypp(right) - ypp(left)) / (6.0*h))));
		ypval = (y(right) - y(left)) / h - (ypp(right) / 6.0 + ypp(left) / 3.0)*h + dt*(ypp(left) + dt*(0.5*(ypp(right) - ypp(left)) / h));
	...
	void CalculateNormalForce(int StartFlag, int NumElemsX, ChMatrixDynamic<double> DisFlex, ChMatrixDynamic<double> VelFlex, ChMatrixDynamic<double> &ContactFRC, int &SequentialCheck, int &NumContactNode)
	{
		double Kg;
		double Cg;
		double DeltaDis;
	...
	void LuGre_SteadyState(int NumElemsX, int NumElemsY, ChMatrixDynamic<double> DisRigid, ChMatrixDynamic<double> VelRigid, ChMatrixDynamic<double> DisFlex, ChMatrixDynamic<double> VelFlex, ChMatrixDynamic<double> &ContactFRC, ChMatrixDynamic<double> &StockSlipVel1, double & StockROMG1)
	{
		double MinRot = 0.0;
		double MaxRot = 0.0;
		double rot = 0.0;
	...
	// RxOmg: Measure of an average linear velocity (along the forward direction) of the nodes in the entire loop
		...
		CalculateNormalForce(StartFlag, NumElemsX, DisFlex, VelFlex, ContactFRC, SequentialCheck, NumContactNode);
			...
			if (NumContactNode >= 2 && SequentialCheck < 3) // If SequentialCheck >= 3, discontinuity within contact patch (buckling)
			{
				...
				// Estimate actual trailing/leading point from the nodes that surround the contact patch boundary (one in one out)
				RearContactPoint(0) = NodeTransPos(0, 0) - sqrt((NodeTransPos(1, 0) - NodeTransPos(0, 0))*(NodeTransPos(1, 0) - NodeTransPos(0, 0))) / 2.0;
				RearContactPoint(1) = NodeTransPos(0, 1) - sqrt((NodeTransPos(1, 1) - NodeTransPos(0, 1))*(NodeTransPos(1, 1) - NodeTransPos(0, 1))) / 2.0;
				RearContactPoint(2) = ContactLine;
				FrontContactPoint(0) = NodeTransPos(NumContactNode - 1, 0) + sqrt((NodeTransPos(NumContactNode - 1, 0) - NodeTransPos(NumContactNode - 2, 0))*(NodeTransPos(NumContactNode - 1, 0) - NodeTransPos(NumContactNode - 2, 0))) / 2.0;
				FrontContactPoint(1) = NodeTransPos(NumContactNode - 1, 1) + sqrt((NodeTransPos(NumContactNode - 1, 1) - NodeTransPos(NumContactNode - 2, 1))*(NodeTransPos(NumContactNode - 1, 1) - NodeTransPos(NumContactNode - 2, 1))) / 2.0;
				...
				Length = abs(RearContactPoint(0) - FrontContactPoint(0));
				TempR = Length / double(NumLuGreZ - 1); // Distance between LuGre discretization points (NumLuGreZ: Number of "Lugre" elements within a strip)
				...
				for (int i = 0; i < NumLuGreZ; i++)
				{
					...
					spline_cubic_val(NumContactNode, NodeTransPosX, NodeTransPosY, ypp, ZetaPos(i, 0), ZetaPos(i, 1), Dummy, Dummy);
				}
				...
				for (int i = 0; i < NumLuGreZ; i++)
				{
					if (i == 0)
					{
						Calculate_dLength(ZetaPos(i + 1, 0), ZetaPos(i + 1, 1), ZetaPos(i, 0), ZetaPos(i, 1), ZetaPos(i + 1, 0), ZetaPos(i + 1, 1), dLength(i));
					}
				...
				for (int i = 0; i < NumContactNode; i++)
				{
					if (i == 0)
					{
						Calculate_dLength(NodeTransPos(i + 1, 0), NodeTransPos(i + 1, 1), NodeTransPos(i, 0), NodeTransPos(i, 1), NodeTransPos(i + 1, 0), NodeTransPos(i + 1, 1), dLength_Node(i));
					}
				...
				for (int i = 0; i < NumLuGreZ; i++)
				{
					if (RearContactPoint(0) <= ZetaPos(i, 0) && ZetaPos(i, 0) <= FrontContactPoint(0))
					{
						...
						spline_cubic_val(NumContactNode + 2, TempNodeInfo1, TempFn, ypp, ZetaPos(i, 0), ZetaFn(i), Dummy, Dummy);
					}
				}
				...
				for (int i = 0; i < NumLuGreZ; i++)
				{
					if (RearContactPoint(0) <= ZetaPos(i, 0) && ZetaPos(i, 0) <= FrontContactPoint(0))
					{
						...
						spline_cubic_val(NumContactNode, NodeTransPosX, NodeTransVelX, ypp, ZetaPos(i, 0), ZetaVr(i, 0), Dummy, Dummy);
						// Slip in Y-direction
						spline_cubic_set(NumContactNode, NodeTransPosX, NodeTransVelY, 2, 0.0, 2, 0.0, ypp);
						spline_cubic_val(NumContactNode, NodeTransPosX, NodeTransVelY, ypp, ZetaPos(i, 0), ZetaVr(i, 1), Dummy, Dummy);
					}
				}
				...
				if (RearContactPoint(0) <= ZetaPos(0, 0) && ZetaPos(0, 0) <= FrontContactPoint(0))
				{
					...
					Gx_Func = G_Func*sqrt((ZetaVr(0, 0) / sqrt(ZetaVr(0, 0)*ZetaVr(0, 0) + ZetaVr(0, 1)*ZetaVr(0, 1)))*(ZetaVr(0, 0) / sqrt(ZetaVr(0, 0)*ZetaVr(0, 0) + ZetaVr(0, 1)*ZetaVr(0, 1))));
					...
					Gy_Func = G_Func*sqrt((ZetaVr(0, 1) / sqrt(ZetaVr(0, 0)*ZetaVr(0, 0) + ZetaVr(0, 1)*ZetaVr(0, 1)))*(ZetaVr(0, 1) / sqrt(ZetaVr(0, 0)*ZetaVr(0, 0) + ZetaVr(0, 1)*ZetaVr(0, 1))));
					ParaA = sqrt(RxOmg*RxOmg) / dLength(0);
				...
				for (int i = 1; i < NumLuGreZ; i++)
				{
					if (RearContactPoint(0) <= ZetaPos(i, 0) && ZetaPos(i, 0) <= FrontContactPoint(0))
					{
						...
						Gx_Func = G_Func*sqrt((ZetaVr(i, 0) / sqrt(ZetaVr(i, 0)*ZetaVr(i, 0) + ZetaVr(i, 1)*ZetaVr(i, 1)))*(ZetaVr(i, 0) / sqrt(ZetaVr(i, 0)*ZetaVr(i, 0) + ZetaVr(i, 1)*ZetaVr(i, 1))));
						...
						dZLuGre(i, 0) = 0.0;
						//dZLuGre(i, 0) = ZetaVr(i, 0) + ParaA*ZLuGre(i - 1, 0) + ParaB*ZLuGre(i, 0); // For transient LuGre

						Gy_Func = G_Func*sqrt((ZetaVr(i, 1) / sqrt(ZetaVr(i, 0)*ZetaVr(i, 0) + ZetaVr(i, 1)*ZetaVr(i, 1)))*(ZetaVr(i, 1) / sqrt(ZetaVr(i, 0)*ZetaVr(i, 0) + ZetaVr(i, 1)*ZetaVr(i, 1))));
						ParaA = sqrt(RxOmg*RxOmg) / dLength(i);
						ParaB = -(sgm0_y*sqrt(ZetaVr(i, 1)*ZetaVr(i, 1)) / Gy_Func + ParaA);
						ZLuGre(i, 1) = -(ZetaVr(i, 1) + ParaA*ZLuGre(i - 1, 1)) / ParaB;
						dZLuGre(i, 1) = 0.0;
						//dZLuGre(i, 1) = ZetaVr(i, 1) + ParaA*ZLuGre(i - 1, 1) + ParaB*ZLuGre(i, 1); // For transient LuGre
					}
				}
	...
	virtual void ComputeQ(ChState*      state_x, ///< state position to evaluate Q
		...
		if (state_x && state_w) {
			...
			LuGre_SteadyState(loadables.size(), 24, RigidPos, RigidVel, FlexPos, FlexVel, TempContactFRCE, TempSlipVel, TempROMG);
...
int main(int argc, char* argv[]) {
	...
	ChMatrixNM<double, 15, 2> LayPROP;
	...
	int NumCont = 0;
	double ContactZ = -0.02; // Vertical location of the ground

	// Create all the nodes of the tire
	for (int i = 0; i < TotalNumNodes; i++)
	{
		...
		node->SetMass(0.0);	// Set node mass
		if (COORDFlex(i, 2) < ContactZ)
		{
			NumCont++; // Initial estimate of contact nodes
		}

		my_mesh->AddNode(node); // Add node to system 
	}
	// Check position of a bottom node
	...
	double timestep = 0.00025; // Minimum recommended time step
	int LayerHist = 0;
	...
	// Apply gravity along the negative Z axis
	...
	// LuGre Load Class initialization (inherits from ChLoadCustomMultiple)
	// Number of Lugre strips: NumElements_y + 1. One strip per node along width
	std::vector<ChSharedPtr<ChLoaderLuGre>> LuGreLoadList(NumElements_y + 1); // One instance od ChLoaderLugre per strip
	for (int i = 0; i < NumElements_y + 1; i++) // Loop over all strips
	{
		std::vector< ChSharedPtr< ChLoadable > > mnodelist;
		for (int inode = 0; inode < (TotalNumNodes / (NumElements_y + 1)); inode++) // Loop over nodes in strip
		{
			ChSharedPtr<ChNodeFEAxyzD> FrictionNode(my_mesh->GetNode((i*TotalNumNodes / (NumElements_y + 1)) + inode).DynamicCastTo<ChNodeFEAxyzD>());
			mnodelist.push_back(FrictionNode); // Add node to each strip's node list 
		}

		LuGreLoadList[i] = ChSharedPtr<ChLoaderLuGre>(new ChLoaderLuGre(mnodelist)); // Add nodelist to each strip's object
		LuGreLoadList[i]->mRim = Rim; // Add pointer to the rim for the calculation of ZLugre (Z parameter of Lugre formulation)
		LuGreLoadList[i]->NumContact = NumCont; // Total number of nodes in contact
		LuGreLoadList[i]->ContactLine = ContactZ; // Give the location of the ground to the load object
		Mloadcontainer->Add(LuGreLoadList[i]); // Add to the container
	}
	...
	my_system.Add(Mloadcontainer); // Add load container to the system

	// Remember to add the mesh to the system!
	my_system.Add(my_mesh);

	// Create constraints for the tire body and rim
	...
	// Constrain the flexible tire to the rigid Rim body. Nodes are numbered by loops in the circumferential X direction 
	for (int i = 0; i < TotalNumNodes; i++)
	{
	...
	// Output ime history of contact forces per nodal coordinate
	if (output4)
	{
		fprintf(outputfile4, "%15.7e  ", my_system.GetChTime()); // Current time

		for (int i = 0; i < NumElements_y + 1; i++)	// Loop over nodes in the width direction
		{
			LuGreLoadList[i]->NumContact = NumCont;
			NetContact.x += LuGreLoadList[i]->NetContactForce.x;
			NetContact.y += LuGreLoadList[i]->NetContactForce.y;
			NetContact.z += LuGreLoadList[i]->NetContactForce.z;

			for (int j = 0; j < NumElements_x; j++) // Loop of circumferential direction
			{
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(0, j));
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(1, j));
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(2, j));
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(3, j));
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(4, j));
				fprintf(outputfile4, "%15.7e  ", LuGreLoadList[i]->NodeContactForce(5, j));
			}
		}
		fprintf(outputfile4, "\n  ");
	}
...
//	FILE* outputfile1;
//	FILE* outputfile2;
//	double COORDFLEX[18000];
//	double VELFLEX[18000];
//	double Force[18000];
//	double time = 0.0;
//
//	inputfile = fopen("OutPutBody1.txt", "r");
//	printf("Open QSOL0_All0.txt\n");
//	if (inputfile == NULL){
//		printf("Restart Information not found!\n");
//		system("pause");
...
//	}
//
//	outputfile1 = fopen("OutPutBody1Split.txt", "w");
//	outputfile2 = fopen("StockContactForceSplit.txt", "w");
//
//	while (time < 0.2)
//	{
//		time = 0.0;
...
//		{
//			COORDFLEX[i] = 0.0;
//			fscanf(inputfile, "%lf ", &COORDFLEX[i]);
//		}
//		for (int i = 0; i < 18000; i++)
//		{
//			Force[i] = 0.0;
//			if (time != 0.0)
//			{
//				fscanf(inputfile, "%lf ", &Force[i]);
//			}
//		}
//		for (int i = 0; i < 18000; i++)
//		{
//			VELFLEX[i] = 0.0;
//			fscanf(inputfile, "%lf ", &VELFLEX[i]);
//		}
//		fscanf(inputfile, "\n");
//
//
//		fprintf(outputfile1, "%15.7e  ", time);
//		fprintf(outputfile2, "%15.7e  ", time);
//		for (int i = 0; i < 18000; i++)
//		{
//			fprintf(outputfile1, "%15.7e  ", COORDFLEX[i]);
//		}
//		for (int i = 0; i < 18000; i++)
//		{
//			fprintf(outputfile1, "%15.7e  ", VELFLEX[i]);
//		}
//		for (int i = 0; i < 18000; i++)
//		{
//			fprintf(outputfile2, "%15.7e  ", Force[i]);
//		}
//		fprintf(outputfile1, "\n  ");
//		fprintf(outputfile2, "\n  ");
//
